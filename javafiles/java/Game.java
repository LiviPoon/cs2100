...
public class Game{
        ...
        while(!validCategory){
            try{
                ...
                for (CategoryValue cv: CategoryValue.values()){
                    System.out.printf("%d: %s, %d points%n", cv.getValue(), cv, players.get(player).getScoreCard().getEvaluation(cv, dRoll));
                }
    ...
    /**
     * method playGame is as the name suggests, its responsible for the running the entire game.
     */
